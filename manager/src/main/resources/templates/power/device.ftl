        ...
        $(function () {
            ...
            layui.use(['layer','form','layedit','laydate'], function(){
                ...
                form.on('submit(addeditsubmitfilter)', function(data) {
                    ...
                    $.ajax({
                        type: "POST",
                        url:"admin/device/addupdatedevice",
                        data:$('#addeditformid').serialize(),
                        async: false,
                        error: function(request) {
                            layer.msg("与服务器连接失败/(ㄒoㄒ)/~~");
                        },
        ...
        function pageInit() {
            ...
            jQuery("#list2").jqGrid(
                    {
                        ...
                        emptyrecords: "当前无记录",
                        colNames: ['ID', '设备名', '设备类型','设备通道号', '设备编号', '上线时间','最近一次操作时间' ,'用户集'],//jqGrid的列显示名字
                        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
                            {name: 'id', index: 'id', width: 100, sortable: true, search: false},
                            {name: 'deviceName', index: 'deviceName', width: 180, sortable: false,search: true,
                                //被该列搜索时的搜索条件有哪些
                            ...
                            {name: 'deviceType', index: 'deviceType', width: 200, sortable: true, search: false},
                            {name: 'deviceChannel', index: 'deviceChannel', width: 180, sortable: false, search: false},
                            {name: 'deviceNo', index: 'deviceNo', width: 180, sortable: false, search: false},
                            {name: 'ctime', index: 'ctime', width: 180, sortable: true, search: false},
                            {name: 'utime', index: 'utime', width: 180, sortable: true, search: false},
                            {name: 'users', index: 'users', width: 180, sortable: false, search: false}

                        ],
                        //如果使用自定义按钮点击事件的方式进行记录增删改操作的话下可以去掉
                        //editurl: "admin/user/adduser",{name: 'ctime', index: 'ctime', width: 180, sortable: false, search: false}
            ...
            $("#edit").click(function () {
                ...
                if (id) {
                    ...
                    $.ajax({
                        type: "POST",
                        url:"admin/device/selectDeviceById",
                        data:{id:ret.id},
                        async: false,
                        error: function(request) {
                            layer.msg("与服务器连接失败/(ㄒoㄒ)/~~");
                        ...
                        success: function(data) {
                            ...
                            if(data.state=='success'){
                                ...
                                $("#deviceName").val(data.tdevice.deviceName);
                                $("#deviceType").val(data.tdevice.deviceType);
                                $("#deviceChannel").val(data.tdevice.deviceChannel);
                                $("#deviceNo").val(data.tdevice.deviceNo);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if(data.state=='success'){
                                ...
                                var userList=[];
                                userList=data.userList;//该记录已经拥有的记录集合
                                var notinuserlist=[];
                                notinuserlist=data.notinuserlist;//该记录尚未拥有的记录集合

                                var strs="";
                                $.each(userList, function (n, value) {//n从0开始自增+1；value为每次循环的单个对象
                                    layer.msg(value.trueName+value.id);

                                    strs+='<input type="checkbox" name="user" title="'+value.trueName+'" value="'+value.id+'"  checked="checked">';
                                });
                                $.each(notinuserlist, function (n, value) {
                                    strs+='<input type="checkbox" name="user" title="'+value.trueName+'"  value="'+value.id+'" >';
                                });
...
<div class="layui-btn-group">
    ...
    <button class="layui-btn" id="editrole">设置用户</button>
    ...
    <form class="layui-form" action="" id="addeditformid">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="deviceName" name="deviceName" lay-verify="userName" autocomplete="off" placeholder="请输入设备名" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">设备类型</label>
            <div class="layui-input-block">
                <select id="deviceType" name= "deviceType"lay-verify="required" class="layui-select"  >
                ...
                <#--<input type="text" id="password" name="password" lay-verify="password" autocomplete="off" placeholder="请输入设备类型" class="layui-input">-->
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">设备通道号</label>
            <div class="layui-input-block">
                <input type="text" id="deviceChannel" name="deviceChannel" autocomplete="off" placeholder="请输入设备通道号" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">设备编号</label>
            <div class="layui-input-block">
                <input type="text" id="deviceNo" name="deviceNo" autocomplete="off" placeholder="请输入设备编号" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
...
<div id="editroleformdivid" hidden="" class="layui-fluid" style="margin: 15px;">
    <form class="layui-form" action="" id="editroleformid">
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">用户复选框</label>
